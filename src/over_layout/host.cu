...
void stencil_on_gpu(float *A_h, float *S_h, float *C)
{
    ...
    printf("row_pos_min:%d row_pos_max:%d col_pos_min:%d col_pos_max:%d\n", row_pos_min, row_pos_max, col_pos_min, col_pos_max);
    //grid形状
    ...
    int shared_mem_size = shared_mem_row_size + shared_mem_col_size + shared_mem_store;
    int shared_mem_row_addr = (Block_N*abs(row_pos_min));//row指针位置  load数据位置
    int shared_mem_col_addr = (Block_M*Block_N + Block_N*(row_pos_max - row_pos_min) + Block_M*abs(col_pos_min));//col指针位置  load数据位置
    int shared_mem_store_addr = (Block_M*Block_N + Block_N*(row_pos_max - row_pos_min)) + (Block_M*Block_N + Block_M*(col_pos_max - col_pos_min));//store指针位置
