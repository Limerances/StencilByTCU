    ...
    int N, int padding)//分块边长大于16是必然的
{
    ...
    // 加载halo区数据
    // halo1
    int halo_offset = offset - halo1_m*size;
    ...
    //halo2
    halo_offset = offset + Block_M*size;
    ...
    //计算halo区
    //置零
    times = (Block_N*Block_M - 1)/(blockDim.x*blockDim.y) + 1;
    #pragma unroll
    for(int i = 0; i < times; ++i)
    {
        int index = i*blockDim.x*blockDim.y + tid;
        if(index < Block_M*Block_N && \
            !(halo3_n <= index%Block_N && index%Block_N < Block_N - halo4_n && halo1_m <= index/Block_N && index/Block_N < Block_M - halo2_m))
        {
            store_data[index] = 0;
        }
    }
    ...
    // __syncthreads();
    //置零然后计算第五区域！！！即内部
    half *halo0 = (half *)(row_data + abs(row_pos_min)*Block_N);
    int halo0_m = Block_M;
    int halo0_n = Block_N;
    times = (Block_N*Block_M - 1)/(blockDim.x*blockDim.y) + 1;
    #pragma unroll
    for(int i = 0; i < times; ++i)
    {
        ...
        if(index < Block_M*Block_N && \
            !(halo3_n <= index%Block_N && index%Block_N < Block_N - halo4_n && halo1_m <= index/Block_N && index/Block_N < Block_M - halo2_m))
        {
            int pos_m = index/Block_N;
            int pos_n = index%Block_N;
            #pragma unroll
            for(int y = 0; y < stencil_shape_M; ++y)
            {
                #pragma unroll
                for(int x = 0; x < stencil_shape_N; ++x)
                {
                    int halo0_y = (pos_m) - stencil_core_M + y;
                    int halo0_x = (pos_n) - stencil_core_N + x;
                    if(0 <= halo0_y && halo0_y < halo0_m && \
                        0 <= halo0_x && halo0_x < halo0_n)
                    {
                        store_data[index] += \
                        __half2float(halo0[halo0_y*halo0_n + halo0_x]) * S_data[y*stencil_shape_N + x];
                    }
                }
            }
        }
    ...
    //计算store_data中和halo1区有联系的区域
    times = (halo1_m*Block_N - 1)/(blockDim.x*blockDim.y) + 1;
    #pragma unroll
    for(int i = 0; i < times; ++i)
    {
        if(i*blockDim.x*blockDim.y + tid < halo1_m*Block_N)
        {
            int pos_m = (i*blockDim.x*blockDim.y + tid)/Block_N;
            int pos_n = (i*blockDim.x*blockDim.y + tid)%Block_N;
            #pragma unroll
            for(int y = 0; y < stencil_shape_M; ++y)
            {
                #pragma unroll
                for(int x = 0; x < stencil_shape_N; ++x)
                {
                    //(i*blockDim.x*blockDim.y + tid)点相对于halo1区的坐标
                    //0,0相对于(i*blockDim.x*blockDim.y + tid)点的坐标
                    //x,y相对于0,0的坐标
                    int halo1_y = (halo1_m + pos_m) - stencil_core_M + y;
                    int halo1_x = (pos_n) - stencil_core_N + x;
                    if(0 <= halo1_y && halo1_y < halo1_m && \
                        0 <= halo1_x && halo1_x < halo1_n)
                    {
                        store_data[pos_m*Block_N + pos_n] += \
                        __half2float(halo1[halo1_y*halo1_n + halo1_x]) * S_data[y*stencil_shape_N + x];
                    }
                }
            }
        }
    ...
    //计算store_data中和halo2区有联系的区域
    times = (halo2_m*Block_N - 1)/(blockDim.x*blockDim.y) + 1;
    #pragma unroll
    for(int i = 0; i < times; ++i)
    {
        if(i*blockDim.x*blockDim.y + tid < halo2_m*Block_N)
        {
            int pos_m = (i*blockDim.x*blockDim.y + tid)/Block_N;
            int pos_n = (i*blockDim.x*blockDim.y + tid)%Block_N;
            #pragma unroll
            for(int y = 0; y < stencil_shape_M; ++y)
            {
                ...
                for(int x = 0; x < stencil_shape_N; ++x)
                {
                    //(i*blockDim.x*blockDim.y + tid)点相对于halo2区的坐标
                    //0,0相对于(i*blockDim.x*blockDim.y + tid)点的坐标
                    //x,y相对于0,0的坐标
                    int halo2_y = (-halo2_m + pos_m) - stencil_core_M + y;
                    int halo2_x = (pos_n) - stencil_core_N + x;
                    if(0 <= halo2_y && halo2_y < halo2_m && \
                        0 <= halo2_x && halo2_x < halo2_n)
                    {
                        store_data[(Block_M - halo2_m + pos_m)*Block_N + pos_n] += \
                        __half2float(halo2[halo2_y*halo2_n + halo2_x]) * S_data[y*stencil_shape_N + x];
                    }
                }
            }
        }
    ...
    //计算store_data中和halo3区有联系的区域
    times = (Block_M*halo3_n - 1)/(blockDim.x*blockDim.y) + 1;
    #pragma unroll
    for(int i = 0; i < times; ++i)
    {
        if(i*blockDim.x*blockDim.y + tid < Block_M*halo3_n)
        {
            int pos_m = (i*blockDim.x*blockDim.y + tid)/halo3_n;
            int pos_n = (i*blockDim.x*blockDim.y + tid)%halo3_n;
            #pragma unroll
            for(int y = 0; y < stencil_shape_M; ++y)
            {
                #pragma unroll
                for(int x = 0; x < stencil_shape_N; ++x)
                {
                    //(i*blockDim.x*blockDim.y + tid)点相对于halo3区的坐标
                    //0,0相对于(i*blockDim.x*blockDim.y + tid)点的坐标
                    //x,y相对于0,0的坐标
                    int halo3_y = (halo1_m + pos_m) - stencil_core_M + y;
                    int halo3_x = (halo3_n + pos_n) - stencil_core_N + x;
                    if(0 <= halo3_y && halo3_y < halo3_m && \
                        0 <= halo3_x && halo3_x < halo3_n)
                    {
                        store_data[pos_m*Block_N + pos_n] += \
                        __half2float(halo3[halo3_y*halo3_n + halo3_x]) * S_data[y*stencil_shape_N + x];
                    }
                }
            }
        }
    ...
    //计算store_data中和halo4区有联系的区域
    times = (Block_M*halo4_n - 1)/(blockDim.x*blockDim.y) + 1;
    #pragma unroll
    for(int i = 0; i < times; ++i)
    {
        if(i*blockDim.x*blockDim.y + tid < Block_M*halo4_n)
        {
            int pos_m = (i*blockDim.x*blockDim.y + tid)/halo4_n;
            int pos_n = (i*blockDim.x*blockDim.y + tid)%halo4_n;
            #pragma unroll
            for(int y = 0; y < stencil_shape_M; ++y)
            {
                #pragma unroll
                for(int x = 0; x < stencil_shape_N; ++x)
                {
                    //(i*blockDim.x*blockDim.y + tid)点相对于halo4区的坐标
                    //0,0相对于(i*blockDim.x*blockDim.y + tid)点的坐标
                    //x,y相对于0,0的坐标
                    int halo4_y = (halo1_m + pos_m) - stencil_core_M + y;
                    int halo4_x = (-halo4_n + pos_n) - stencil_core_N + x;
                    if(0 <= halo4_y && halo4_y < halo4_m && \
                        0 <= halo4_x && halo4_x < halo4_n)
                    {
                        store_data[pos_m*Block_N + Block_N - halo4_n + pos_n] += \
                        __half2float(halo4[halo4_y*halo4_n + halo4_x]) * S_data[y*stencil_shape_N + x];
                    }
                }
            }
        }
