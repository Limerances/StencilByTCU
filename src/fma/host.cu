...
void stencil_on_gpu(float *A_h, float *S_h, float *C)
{
    ...
    int shared_mem_store = sizeof(float)*(Block_M*Block_N);//储存分块中用WMMA计算得到的小块，采用float存储；只有单个Warp计算且分块大小为WMMA规定大小才不需要这个
    int shared_mem_halo = sizeof(half)*((stencil_shape_M - 1)*(Block_N + stencil_shape_N - 1) + \
                                        (stencil_shape_N - 1)*(Block_M + stencil_shape_M - 1));//halo区域
    int shared_mem_S = sizeof(float)*(stencil_shape_M*stencil_shape_N);//stencil区域

    int shared_mem_size = shared_mem_row_size + shared_mem_col_size + shared_mem_store + shared_mem_halo + shared_mem_S;

    int shared_mem_col_addr = (Block_M*Block_N + Block_N*(row_pos_max - row_pos_min));//col指针位置
    int shared_mem_store_addr = shared_mem_col_addr + (Block_M*Block_N + Block_M*(col_pos_max - col_pos_min));//store指针位置
    int shared_mem_halo_addr = shared_mem_store_addr + 2*(Block_M*Block_N);//halo指针位置
    int shared_mem_S_addr = shared_mem_halo_addr + ((stencil_shape_M - 1)*(Block_N + stencil_shape_N - 1) + \
    (stencil_shape_N - 1)*(Block_M + stencil_shape_M - 1));//stencil指针位置
