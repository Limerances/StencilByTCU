    ...
    int N, int padding)//分块边长大于16是必然的
{
    ...
    for(int i = 0; i < stencil_part_size; ++i)
    {
        if(stencil_part_type[i] == 0)// 行，即计算 halo3/halo4
        {
            ...
            #pragma unroll
            for(int col = 0; col < stencil_core_N; ++col)
            {
                #pragma unroll
                for(int num_per_col = 0; num_per_col <= col; ++num_per_col)
                {
                    times = (Block_M - 1)/(blockDim.x*blockDim.y) + 1;
                    #pragma unroll
                    for(int j = 0; j < times; ++j)
                    {
                        int new_id = j*blockDim.x*blockDim.y + tid;
                        if(new_id < Block_M)
                        {
                            store_data[new_id*Block_N + num_per_col] += \
                            __half2float(halo3[col + (halo1_m + new_id + stencil_part_pos[i])*halo3_n])*\
            ...
            #pragma unroll
            for(int col = 0; col < stencil_shape_N - stencil_core_N - 1; ++col)
            {
                #pragma unroll
                for(int num_per_col = 0; num_per_col <= col; ++num_per_col)
                {
                    ...
                    for(int j = 0; j < times; ++j)
                    {
                        ...
                        if(new_id < Block_M)
                        {
                            store_data[new_id*Block_N + Block_N - 1 - num_per_col] += \
                            __half2float(halo4[halo4_n - 1 - col + (halo1_m + new_id + stencil_part_pos[i])*halo4_n])*\
        ...
        else
        {
            ...
            for(int row = 0; row < stencil_core_M; ++row)
            {
                #pragma unroll
                for(int num_per_row = 0; num_per_row <= row; ++num_per_row)
                {
                    times = (Block_N - 1)/(blockDim.x*blockDim.y) + 1;
                    #pragma unroll
                    for(int j = 0; j < times; ++j)
                    {
                        int new_id = j*blockDim.x*blockDim.y + tid;
                        if(new_id < Block_N)
                        {
                            store_data[num_per_row*Block_N + new_id] += \
                            __half2float(halo1[stencil_core_N + new_id + stencil_part_pos[i] + row*halo1_n])*\
                            S_data[s_index + (row - num_per_row)*stencil_shape_N];
                        }
                    }
                }
            }
            for(int row = 0; row < stencil_shape_M - stencil_core_M - 1; ++row)
            {
                #pragma unroll
                for(int num_per_row = 0; num_per_row <= row; ++num_per_row)
                {
                    ...
                    for(int j = 0; j < times; ++j)
                    {
                        ...
                        if(new_id < Block_N)
                        {
                            store_data[(Block_M - 1 - num_per_row)*Block_N + new_id] += \
                            __half2float(halo2[stencil_core_N + new_id + stencil_part_pos[i] + (halo2_m - 1 - row)*halo2_n])*\
