...
void stencil_on_gpu(float *A_h, float *S_h, float *C)
{
    ...
    dim3 grid(MESH_SIZE/(Block_N*Tile_X), MESH_SIZE/(Block_M));
    ...
    //设置三块共享内存的大小
    int row_size = Max(Block_M*Block_N + Block_N*(row_pos_max - row_pos_min),2*Block_M*Block_N);//为了复用store_data，所以取大值
    int shared_mem_row_size = sizeof(half)*row_size;//行拆分的stencil所计算的网格
    int halo_size = (stencil_shape_N - 1)*(Block_M + stencil_shape_M - 1);//halo区域
    halo_size = (halo_size + 1)/2*2;//保证是偶数
    int shared_mem_halo = sizeof(half)*(halo_size);//halo区域
    int shared_mem_S = sizeof(float)*(stencil_shape_M*stencil_shape_N);//stencil区域

    int shared_mem_size = shared_mem_row_size + shared_mem_halo + shared_mem_S;

    int shared_mem_halo_addr = row_size;//halo指针位置
    int shared_mem_S_addr = shared_mem_halo_addr + halo_size;//stencil指针位置
