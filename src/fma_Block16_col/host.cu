...
void stencil_on_gpu(float *A_h, float *S_h, float *C)
{
    ...
    //设置三块共享内存的大小
    int col_size = Max(Block_M*Block_N + Block_M*(col_pos_max - col_pos_min),2*Block_M*Block_N);//为了复用store_data，所以取大值
    int shared_mem_col_size = sizeof(half)*col_size;//行拆分的stencil所计算的网格
    int halo_size = (stencil_shape_M - 1)*(Block_N + stencil_shape_N - 1);
    ...
    int shared_mem_size = shared_mem_col_size + shared_mem_halo + shared_mem_S;

    int shared_mem_halo_addr = col_size;//halo指针位置
